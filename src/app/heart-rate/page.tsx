          ...
          <Card>
            <div className="mb-5 flex items-start justify-between gap-4">
              ...
              <div className="rounded-2xl bg-brand-50 px-3 py-2 text-right dark:bg-slate-900/30">
                ...
                <p className="text-lg font-black text-brand-700 dark:text-brand-300">{latestEntry ? `${latestEntry.bpm} BPM` : "—"}</p>
              </div>
            </div>
            ...
            <form className="mt-6 space-y-3" onSubmit={submitHeartRate}>
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label htmlFor="heart-rate-bpm" className="mb-1 block text-sm font-semibold text-slate-600 dark:text-slate-300">
                    Heart rate (BPM)
                  </label>
                  <input
                    id="heart-rate-bpm"
                ...
                <div>
                  <label htmlFor="resting-bpm" className="mb-1 block text-sm font-semibold text-slate-600 dark:text-slate-300">
                    Resting BPM
                  </label>
                  <input
                    id="resting-bpm"
              ...
              <div>
                <label htmlFor="hrv-ms" className="mb-1 block text-sm font-semibold text-slate-600 dark:text-slate-300">
                ...
                heartRateEntries.slice(0, 6).map((entry) => (
                  <div key={entry.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-lg font-black text-slate-900 dark:text-slate-100">{entry.bpm} BPM</p>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                      {entry.hrvMs ? <span className="rounded-full bg-white px-3 py-1 dark:bg-slate-800/70">HRV {entry.hrvMs} ms</span> : null}
                    </div>
                  </div>
                ))
              ) : (
